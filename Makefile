...
test-unit:
	mkdir -p $(REPORTS_DIR)
	poetry run pytest tests/unit/ --cov=terok_sandbox --cov-report=term-missing --cov-report=xml:$(COVERAGE_XML) --cov-report=json:$(COVERAGE_JSON) --junitxml=$(UNIT_JUNIT_XML) -o junit_family=legacy
