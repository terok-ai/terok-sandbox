...
Concrete implementations: :class:`.podman.PodmanRuntime` (default),
:class:`.null.NullRuntime` (tests / dry-run).  A future ``KrunRuntime`` for
...
layer (see :class:`terok_sandbox.sandbox.Sandbox`).
"""
...
class ExecResult:
    """Outcome of :meth:`ContainerRuntime.exec`.

    Backend-neutral so a future SSH-over-vsock krun backend can fill it from
    an SSH response without pretending to be a :class:`subprocess.CompletedProcess`.
    """
...
class ContainerRemoveResult:
    """Per-container outcome from :meth:`ContainerRuntime.force_remove`."""
...
class Container(Protocol):
    """Handle to a container managed by a :class:`ContainerRuntime`.
    ...
    def wait(self, timeout: float | None = None) -> int:
        ...
        Raises :class:`TimeoutError` when *timeout* elapses.
        """
        ...
    ...
    def login_command(self, *, command: tuple[str, ...] = ()) -> list[str]:
        """Return an argv suitable for :func:`os.execvp` to attach interactively.
...
class ContainerRuntime(Protocol):
    ...
    def container(self, name: str) -> Container:
        ...
        Does not verify existence; call :attr:`Container.state` for that.
        """
        ...
    ...
    def image(self, ref: str) -> Image:
        ...
        Does not verify existence; call :meth:`Image.exists` for that.
        """
        ...
    ...
    def force_remove(self, containers: list[Container]) -> list[ContainerRemoveResult]:
        ...
        one :class:`ContainerRemoveResult` per input.  An already-absent
    ...
    def reserve_port(self, host: str = "127.0.0.1") -> PortReservation:
        ...
        The returned :class:`PortReservation` exposes the port number via
