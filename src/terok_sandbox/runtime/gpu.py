...
- **amd** — CDI (``amd.com/gpu``, generated by ``amd-ctk``) when
  present; otherwise the ROCm-documented ``/dev/kfd`` plus the AMD
  render nodes.  The raw recipe is also what works on older podman
  releases (< 4.1) that predate CDI.
- **intel** — CDI (``intel.com/gpu``) when present; otherwise the
  Intel render nodes — the oneAPI/NEO stack needs nothing else.

Raw DRM grants are vendor-scoped: only the granted vendor's render
nodes are mounted, never the whole ``/dev/dri`` (a mixed host's other
GPUs — or its BMC's display device — stay invisible).  Each granted
node's ``/dev/dri/by-path`` symlink rides along read-only so
...
def _amd_args(cdi_kinds: frozenset[str], devices: tuple[int, ...] | None = None) -> list[str]:
    ...
    The raw recipe mounts ``/dev/kfd`` plus the AMD render nodes —
    vendor-scoped, never the whole ``/dev/dri`` — and only the selected
    ones for indexed grants.  KFD's topology still lists every GPU, but
    a card whose render node is absent cannot be driven.
    """
    ...
    if _KFD_DEVICE.exists():
        pairs = _selected_render_pairs("amd", _AMD_PCI_VENDOR, devices)
        if devices is not None:
            _warn_best_effort("amd")
        node_args = [arg for _, node in pairs for arg in ("--device", str(node))]
        return [
            "--device",
            str(_KFD_DEVICE),
            *node_args,
            *_by_path_link_args(pairs),
            *_KEEP_GROUPS_ARGS,
        ]
    raise GpuConfigError(
...
def _intel_args(cdi_kinds: frozenset[str], devices: tuple[int, ...] | None = None) -> list[str]:
    """Intel passthrough — CDI when generated, Intel render nodes otherwise."""
    if _INTEL_GPU_KIND in cdi_kinds:
        refs: tuple[str, ...] = ("all",) if devices is None else tuple(str(i) for i in devices)
        device_args = [arg for ref in refs for arg in ("--device", f"{_INTEL_GPU_KIND}={ref}")]
        return [*device_args, *_KEEP_GROUPS_ARGS]
    if _has_render_node(_INTEL_PCI_VENDOR):
        pairs = _selected_render_pairs("intel", _INTEL_PCI_VENDOR, devices)
        if devices is not None:
            _warn_best_effort("intel")
        node_args = [arg for _, node in pairs for arg in ("--device", str(node))]
        return [*node_args, *_by_path_link_args(pairs), *_KEEP_GROUPS_ARGS]
    raise GpuConfigError(
...
def _resolve_all(cdi_kinds: frozenset[str], *, strict: bool = True) -> tuple[GpuVendor, ...]:
    ...
    usable = {
        ...
        "amd": _AMD_GPU_KIND in cdi_kinds
        or (_KFD_DEVICE.exists() and _has_render_node(_AMD_PCI_VENDOR)),
        "intel": _INTEL_GPU_KIND in cdi_kinds or _has_render_node(_INTEL_PCI_VENDOR),
    }
...
def _has_render_node(pci_vendor: str) -> bool:
    """True when any DRM render node belongs to the given PCI vendor."""
    return any(
        _safe_read(node / "device" / "vendor").strip() == pci_vendor
...
def _selected_render_pairs(
    vendor: GpuVendor, pci_vendor: str, devices: tuple[int, ...] | None
) -> list[tuple[str, Path]]:
    """The vendor's granted ``(pci_address, node)`` pairs — all of them,
    or the indexed subset, failing loudly on out-of-range indices."""
    pairs = _vendor_render_nodes(pci_vendor)
    if not pairs:
        raise GpuConfigError(
            f"run.gpus requests '{vendor}' but no {vendor} render node was found "
            f"under {_DRM_SYSFS} — the kernel driver looks loaded, yet exposes "
            "no DRM render device."
        )
    if devices is None:
        return pairs
    if bad := [i for i in devices if i >= len(pairs)]:
        available = ", ".join(f"{vendor}:{i}={addr}" for i, (addr, _) in enumerate(pairs))
        raise GpuConfigError(
            f"run.gpus selects {vendor} device(s) {bad} but this host has "
            f"{len(pairs)} {vendor} device(s): {available}"
        )
    return [pairs[i] for i in devices]
...
def _by_path_link_args(pairs: list[tuple[str, Path]]) -> list[str]:
    """Read-only binds of the granted nodes' ``by-path`` entries.

    ``--device`` replicates the device nodes but not the PCI-addressed
    ``/dev/dri/by-path`` symlinks; binding them keeps Intel NEO's
    primary (PCI-ordered) enumeration path working and makes multi-GPU
    device identity legible inside the container.  Bound per granted
    node — never the whole directory, whose other entries would dangle —
