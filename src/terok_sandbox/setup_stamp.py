...
Package versions are read via :func:`importlib.metadata.version` —
no subprocess, no parsing, sub-millisecond per package.  Total cost
of :func:`needs_setup` on a happy path: one ``Path.is_file``, one
...
class SetupVerdict(Enum):
    """Result of :func:`needs_setup` — five possible states a launch can be in."""
...
def needs_setup() -> SetupVerdict:
    ...
    See :class:`SetupVerdict` for the five possible outcomes.  Designed
...
def _read_stamp(path: Path) -> dict[str, str]:
    ...
    Raises :class:`ValueError` if the schema version doesn't match — a
