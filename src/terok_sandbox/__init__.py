...
__all__ = [
    ...
    "is_apparmor_enabled",
    # Backend-agnostic hardening installer
...
# The hardening install command is backend-agnostic; the historically-
# named ``selinux_*`` and ``apparmor_*`` aliases are kept as a
# back-compat layer so existing terok sickbay imports keep working
# without a separate "they're the same string" explainer.
selinux_install_command = hardening_install_command
apparmor_install_command = hardening_install_command
