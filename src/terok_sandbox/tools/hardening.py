...
from .._util._apparmor import is_apparmor_enabled, loaded_confined_profiles
from .._util._fs import systemd_user_unit_dir
from .._util._selinux import is_selinux_enabled, loaded_confined_domains
...
def install() -> None:
    ...
    if is_apparmor_enabled() and shutil.which("apparmor_parser"):
...
def remove() -> None:
    ...
    SELinuxContext attachment) then unloads modules.
    """
    _remove_dropins()
    _restart_active_units()
    ...
    if is_selinux_enabled() and shutil.which("semodule"):
...
def _remove_dropins() -> None:
    ...
    for unit, _, _ in SERVICE_UNITS:
        ...
        try:
            ...
            continue
        # Preserve sibling drop-ins (auditd, namespacing) installed by
        # other layers — rmdir is a no-op when the .d/ still has them.
        try:
            f.parent.rmdir()
        except OSError:
            pass
...
def _restart_active_units() -> None:
    ...
    for unit, _, _ in SERVICE_UNITS:
        ...
        if "@" in unit:
            continue
        if _systemctl.query("is-active", "--quiet", unit).returncode == 0:
            _systemctl.run_best_effort("restart", unit)
...
def status() -> None:
    ...
    diagnostics and for the orchestrator in terok itself.
    """
    sel = loaded_confined_domains() if is_selinux_enabled() else ()
    aa = loaded_confined_profiles() if is_apparmor_enabled() else ()
