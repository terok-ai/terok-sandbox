...
def run_supervisor_install_phase() -> bool:
    ...
    * ``<state_root>/hooks/terok-sandbox-supervisor.json`` — the OCI
      hook descriptor.  ``containers.conf`` is patched at install
      time to list ``state_root() / "hooks"`` in ``hooks_dir`` so
      podman scans the canonical terok-owned directory.
...
def run_legacy_install_cleanup_phase() -> bool:
    ...
    * the legacy vault systemd units
      ...
      ``terok-vault-socket.service``);
    * any ``terok-clearance-*.service`` / ``terok-vault-*.service`` /
      ``terok-vault-*.socket`` files lingering in the user's systemd
      unit directory (catches renamed variants from prior alphas);
...
def _sweep_legacy_unit_files() -> None:
    """Unlink stray ``terok-clearance-*`` / ``terok-vault-*`` user unit files.
...
def _unlink_legacy_runtime_sockets() -> None:
    ...
    # ``<rt>/<name>`` is left over from an older install.  Gate is
    # excluded — it stays a host-singleton (Phase B will move it).
    for name in ("vault.sock", "ssh-agent.sock"):
        try:
