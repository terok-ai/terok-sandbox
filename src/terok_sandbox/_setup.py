...
# ── Bridge phase (cross-cutting integration: shield → clearance) ──────


def run_bridge_install_phase() -> bool:
    """Install the shield→clearance event bridge.

    Conceptually a *wire*, not a service: the OCI hook pair and reader
    script that carry NFLOG events from shield's per-container netns
    over to the clearance hub.  Owned at the composer layer (rather
    than in either endpoint phase) so the install's call to
    ``install_bridge_hooks`` (a terok-shield primitive) and its smoke
    check against the on-disk artefacts can both be expressed without
    introducing a Python-import edge between the two endpoint
    packages.

    The endpoints' *runtime* coupling is real — clearance's verdict
    helper execs ``terok-shield`` — but lives in unit files and
    ExecStart lines, not in the Python import graph.  Keeping it that
    way is what lets each package keep an independent test surface.
    Skipped cleanly when ``terok_clearance`` isn't importable: with
    no consumer, the wire would just spawn readers that hit
    ``ECONNREFUSED`` on every connect.
    """
    try:
        import terok_clearance  # noqa: F401 — presence check only
    except ImportError:
        with _stage_line("Shield bridge") as s:
            s.skip("clearance not installed — events would have nowhere to land")
        return True

    from .shield import install_shield_bridge
    ...
    with _stage_line("Shield bridge") as s:
        try:
            ...
            s.fail(str(exc))
            return False
        if not _bridge_artefacts_present():
            s.fail("install returned but bridge files missing on disk")
            return False
        s.ok("installed")
        return True
...
def run_bridge_uninstall_phase() -> bool:
    ...
    try:
        ...
        with _stage_line("Shield bridge") as s:
            s.skip("terok-shield not installed")
        return True
    with _stage_line("Shield bridge") as s:
        try:
...
def _bridge_artefacts_present() -> bool:
    """Smoke check: did the install actually land the files we expected?

    The architectural payoff of having an integration phase: install
    isn't reported green just because ``install_shield_bridge()``
    returned without exception.  Files have to be on disk where the
    OCI hooks runtime will look — both bridge-hook JSONs and the
    reader script at its canonical path.  Catches the class of
    regression where the install function silently no-ops or writes
    ...
    from .shield import USER_HOOKS_DIR

    hooks_dir = Path(USER_HOOKS_DIR).expanduser()
    return (
        reader_script_path().is_file()
        and (hooks_dir / "terok-shield-bridge-createRuntime.json").is_file()
        and (hooks_dir / "terok-shield-bridge-poststop.json").is_file()
    )
