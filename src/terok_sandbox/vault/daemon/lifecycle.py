...
"""[`VaultManager`][terok_sandbox.vault.daemon.lifecycle.VaultManager] — start, stop, status, and pre-task health checks.

Supports systemd socket activation (preferred) and a manual daemon
fallback.  The systemd socket unit listens on both the Unix socket and
the TCP port used by containers; a connection to either triggers the
service.  ``ensure_reachable`` also performs an explicit start as a
belt-and-suspenders measure before task creation.

The status vocabulary (``VaultStatus`` dataclass + ``VaultUnreachableError``)
lives in [`status`][terok_sandbox.vault.daemon.status]; this module
re-exports it so callers that grew up against ``lifecycle.X`` keep
working.
"""
...
from terok_sandbox._util import (
    _systemctl,
    read_pidfile_safely,
    systemd_exec_argv,
...
class VaultManager:
    ...
    def is_daemon_running(self) -> bool:
        ...
        pid = read_pidfile_safely(self._cfg.vault_pid_path)
        if pid is None:
            return False
    ...
    def _systemd_unit_dir() -> Path:
        ...
        from terok_sandbox._util import systemd_user_unit_dir
