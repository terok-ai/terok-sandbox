...
- [`encryption`][terok_sandbox.vault.store.encryption] — six-tier passphrase resolution chain
  (session-unlock file → systemd-creds → keyring → passphrase_command
  → config fallback → interactive prompt) and the SQLCipher open /
  migrate primitives every other store module builds on.
