...
The [`ScopeSocketReconciler`][terok_sandbox.vault.ssh.scope_sockets.ScopeSocketReconciler] watches the DB's ``ssh_keys_version``
...
from .signer import start_ssh_signer_local
...
class ScopeSocketReconciler:
    ...
    * [`start`][terok_sandbox.vault.ssh.scope_sockets.ScopeSocketReconciler.start] performs the initial bind pass and launches the polling
      task.
    * [`stop`][terok_sandbox.vault.ssh.scope_sockets.ScopeSocketReconciler.stop] cancels the poller, closes every bound server, and
