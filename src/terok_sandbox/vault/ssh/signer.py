...
class _DBKeyCache:
    ...
    The class name is kept for API stability; ``_DBKeyCache`` is now a
    thin resolver wrapper over ``_TokenDB``.  ``ssh_keys_version`` is
    no longer consulted because there's nothing to invalidate.
    """
