...
Marker: ``needs_credential_proxy`` — use to select/skip in CI.
"""
...
async def proxy_env(tmp_path: Path, populated_db: CredentialDB):
    """Start a mock upstream and a real proxy, yield (proxy_url, db, upstream_url).

    The proxy routes are rewritten to point at the mock upstream instead of
...
class TestPhantomTokenLifecycle:
    """Story: task creates phantom token, uses it, task ends and token is revoked."""
    ...
    def test_full_token_lifecycle(self, db: CredentialDB) -> None:
        ...
        assert db.lookup_proxy_token(token) is None
...
@pytest.mark.asyncio
class TestProxyForwardsWithRealAuth:
    """Story: container sends request with phantom token → proxy injects real auth."""
    ...
    async def test_claude_api_call(self, proxy_env: dict) -> None:
        ...
        # The upstream saw the REAL token, not the phantom
...
class TestProxyStreaming:
    """Story: proxy streams SSE responses without buffering."""
...
@pytest.mark.asyncio
class TestProxyAfterTokenRevocation:
    """Story: task ends, tokens revoked, proxy rejects subsequent requests."""
    ...
    async def test_revoked_token_rejected_by_proxy(self, proxy_env: dict) -> None:
        ...
        # Revoke
        db.revoke_proxy_tokens("test-project", "task-1")
