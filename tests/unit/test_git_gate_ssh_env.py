    ...
    def test_starts_signer_and_pins_ssh_options(self, tmp_path: Path) -> None:
        ...
        try:
            with (
                ...
                # Gate path under tmp_path so the persistent known_hosts the
                # ssh-env builder touches lands in isolation, not real /tmp.
                gate = _make_gate(gate_path=tmp_path / "mirror" / "proj.git")
                env = gate._ssh_env()
        ...
        # Trust-on-first-use against a persistent known_hosts file instead of
        # disabling host-key verification entirely.
