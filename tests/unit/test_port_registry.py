...
# ---------------------------------------------------------------------------
# Saved-claims reclaim
# ---------------------------------------------------------------------------


class TestSavedPortReclaim:
    """Saved-claims ports are reclaimed even when the bind check would fail."""

    def test_saved_ports_trusted_even_when_bind_fails(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Saved ports are reclaimed despite ``_is_port_free`` returning False.

        This is the core scenario: a prior session saved its ports to
        ``port-claims.json``; on the next resolve, our own service may
        already be listening so ``_is_port_free`` fails — but the saved
        ports are trusted as ours, so the bind check is skipped.
        """
        state = tmp_path / "state"
        state.mkdir()
        (state / reg._CLAIMS_FILENAME).write_text(
    ...
    def test_saved_port_rejected_when_taken_by_other_user(self, tmp_path: Path) -> None:
        """A saved port claimed by another user → SystemExit (fail-closed)."""
        state = tmp_path / "state"
        state.mkdir()
        (state / reg._CLAIMS_FILENAME).write_text(
            json.dumps({"gate": 18750, "proxy": 18751, "ssh_agent": 18752})
    ...
    def test_real_socket_listener_with_saved_port(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """End-to-end: real socket listener + saved-claims reclaim.

        Simulates the actual production scenario with a real listener
        holding the saved gate port.
        """
        ...
        try:
            ...
            ports = reg.resolve_service_ports(None, None, None, state_dir=state)
            assert ports.gate == 18750, "saved port reclaimed despite listener"
        finally:
            listener.close()
