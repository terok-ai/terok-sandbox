...
"""Tests for the credential store and phantom token registry."""
...
class TestProxyTokens:
    """Verify phantom token lifecycle."""
    ...
    def test_lookup_invalid_returns_none(self, db: CredentialDB) -> None:
        ...
        assert db.lookup_proxy_token("nonexistent") is None
    ...
    def test_revoke_removes_all_for_task(self, db: CredentialDB) -> None:
        ...
        assert db.lookup_proxy_token(t2) is None
        assert db.lookup_proxy_token(t3) is not None  # different task

    def test_revoke_idempotent(self, db: CredentialDB) -> None:
        """Revoking tokens for a task with none is a no-op."""
        assert db.revoke_proxy_tokens("nonexistent", "task") == 0
