...
class TestInstallCommand:
    ...
    def test_returns_python_dash_m_tools_path(self) -> None:
        """Returns the ``python -m`` invocation of the hardening tool.

        The hardening installer lives outside the daily user CLI
        (under ``terok_sandbox.tools.hardening`` — a packager-side
        tool, not a feature).  This helper is the single source of
        the invocation string sickbay's ``SELinux policy`` row uses
        to point users at the connectto-allow install path.
        """
        assert install_command() == "python -m terok_sandbox.tools.hardening install"
